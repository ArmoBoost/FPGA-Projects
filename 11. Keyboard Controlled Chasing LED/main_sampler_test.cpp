...
void gsensor_check(SpiCore *spi_p, GpoCore *led_p) {
   ...
   int8_t xraw, yraw, zraw; //these values will be used to determine
   float x, y, z;			//the mag of the tapping

   int8_t xxraw, yyraw, zzraw; //these values will hold the stable
   float xx,yy,zz;			   //mag of the board to prevent random LEDs from glowing
...
int led_chase_keyboard (GpoCore *led, SsegCore *sseg, Ps2Core *ps2){
	static int speed; // How fast the light moves
	char ch; //keyboard key
	static int one=0;
	static int ten=0;
	static int hund=0;
	static int count=0;
	...
	while(count<3){
		...
		if(ps2->get_kb_ch(&ch)){
			...
			&& count==2){
				...
				uart.disp(" ");
				count++;
				speed = (hund*100) + (ten*10) + (one);
				uart.disp("speed: ");
				uart.disp(speed);
				sseg->write_1ptn(sseg->h2s(hund),2);
				sseg->write_1ptn(sseg->h2s(ten),1);
				sseg->write_1ptn(sseg->h2s(one),0);
			}
		}

    }

	return abs(speed - 999);
}

int chase_led(int speed,GpoCore *led_p,Ps2Core *ps2,SsegCore *sseg){
	int i, j;
...
//   timer_check(&led);
   while (1) {
	   if(check==0){
		   speed=led_chase_keyboard(&led,&sseg,&ps2);
	   }
	   check = chase_led(speed,&led,&ps2,&sseg);
//      show_test_id(1, &led);
//      led_check(&led, 16);
//      sw_check(&led, &sw);
//      show_test_id(3, &led);
